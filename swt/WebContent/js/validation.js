		...
		checkPw : function(memPw,memRpw) {
			...
			if(memPw == ""||memPw.length==0){
				...
				if(memRpw!=null||memRpw.length!=0){
					if(memPw==memRpw){
						$(".step_url").eq(2).text(this.resultCode.success_pw.desc).css("display","block").css("color","dodgerblue");
					} else {
						$(".step_url").eq(2).text(this.resultCode.other_pw.desc).css("display","block").css("color","#b30000");
						return false;
					}
				}
